...
async def check_admin(msg: MessageType):
	"""
	Проверяет на наличие прав администратора в группе. Возвращает `False`, если прав лоха у пользователя нет.
	"""

	if not (await BOT.get_chat_member(msg.chat.id, msg.from_user.id)).is_chat_admin():
		await msg.answer("⚠️ Данную команду могут использовать только <b>лохи чата</b>.")
...
async def enable_handler(msg: MessageType):
	...
	await msg.answer(
		...
		"Дебил! Я и так включён, так как не был отключён администраторами беседы до этого."
	)
